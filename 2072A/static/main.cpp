...
// lift motor group
MotorGroup lift_motor_group({15, 16});
// Intake
Motor intake(7);
// Pneumatics
adi::Pneumatics mogo('a', false);
adi::Pneumatics floating('b', false);
adi::Pneumatics claw('c', false);
adi::Pneumatics fold('d', false);
...
void red_Rush()
{
	...
	lift_motor_group.move_absolute(100, 200);
	...
	lift_motor_group.move_absolute(150, 300);
	chassis.waitUntilDone();
	fold.set_value(true);
	lift_motor_group.move_absolute(120, 300);
	claw.set_value(true);
	...
	while (lift_motor_group.get_efficiency() > 10){
		lift_motor_group.move(100);
	}
	lift_motor_group.move(20);
...
void skills()
{
	...
	lift_motor_group.move_absolute(120, 300);
	claw.set_value(true);
	fold.set_value(false);

	chassis.follow(SkillsRunAuto_1_txt, 10, 7, false, true);
	chassis.waitUntilDone();

	mogo.set_value(true);
	intake.move(110);
	chassis.follow(SkillsRunAuto_2_txt, 10, 7, true, true);

	chassis.waitUntil(175);
	mogo.set_value(false);
	lift_motor_group.move_absolute(0, 300);
	fold.set_value(true);
	chassis.waitUntilDone();
...
void opcontrol()
{
	...
	bool claw_tog = true;
	bool fold_tog = true;
	bool floating_tog = true;

	bool mogo_tog_check = false;
	bool claw_tog_check = false;
	bool fold_tog_check = false;
	bool floating_tog_check = false;
	...
	while (true)
	{
		...
		// Pneumatic Toggles
		buttonToggle(master.get_digital(E_CONTROLLER_DIGITAL_L1), mogo, mogo_tog, mogo_tog_check);
		buttonToggle(master.get_digital(E_CONTROLLER_DIGITAL_L2), claw, claw_tog, claw_tog_check);
		buttonToggle(master.get_digital(E_CONTROLLER_DIGITAL_Y), fold, fold_tog, fold_tog_check);
		buttonToggle(master.get_digital(E_CONTROLLER_DIGITAL_RIGHT), floating, floating_tog, floating_tog_check);

		if (master.get_digital(E_CONTROLLER_DIGITAL_R1)) {
			lift_motor_group.move(110);
		} else if (master.get_digital(E_CONTROLLER_DIGITAL_R1)) {
			lift_motor_group.move(-110);
		} else {lift_motor_group.move(0);}
